...
{    
    ...
    while (i<n)
    {
        for (i=i; i<n; i++)
        {
            ...
            // sample value 
            float val = length( texture3D( texture, loc ).rgb );      
            ...
            if (val > th)
            {
                ...
                N[0] = length(color1.rgb) - length(color2.rgb);
                betterVal = max(color1.rgb, color2.rgb);
                color1 = texture3D( texture, loc+vec3(0.0,-step[1],0.0) );
                color2 = texture3D( texture, loc+vec3(0.0,step[1],0.0) );
                N[1] = length(color1.rgb) - length(color2.rgb);
                betterVal = max(max(color1.rgb, color2.rgb), betterVal);
                color1 = texture3D( texture, loc+vec3(0.0,0.0,-step[2]) );
                color2 = texture3D( texture, loc+vec3(0.0,0.0,step[2]) );
                N[2] = length(color1.rgb) - length(color2.rgb);
                betterVal = max(max(color1.rgb, color2.rgb), betterVal);
                N = normalize(N);
