...
from visvis.text.freetype import (
    Face,
    Vector,
    Matrix,
    FT_KERNING_UNFITTED,
...
class FreeTypeAtlas(AtlasTexture):
    ...
    def __init__(self, width=1024, height=1024, depth=1):
        ...
        self.data = np.zeros((self.height, self.width, self.depth), dtype=np.uint8)
...
class FreeTypeFontManager(FontManager):
    ...
    def Compile(self, textObject):
        ...
        fonts = [
            (
                self.GetFont(textObject.fontName, textObject._actualFontSize),
                0,
                False,
                False,
            )
        ...
        for i, charcode in enumerate(tt):
            if not escaped:
                if charcode == "_":
                    font = self.GetFont(
                        textObject.fontName, font.size * 0.7, bold, italic
                    )
        ...
        if False:  # glyph is not None: # Not used
            textObject.width = (
                pen[0] - glyph.advance[0] / 64.0 + glyph.size[0] if tt else 0
            )
    ...
    def _load(self, charcodes, face):
        ...
        for charcode in charcodes:
            ...
            x, y, w, h = self.atlas.get_region(
                width / self.depth + 2 * padmarg, rows + 2 * padmarg
            )
            if x < 0:
                print("Missed !")
                continue
            ...
            for g in self.glyphs.values():
                ...
                kerning = face.get_kerning(
                    g.charcode, charcode, mode=FT_KERNING_UNFITTED
                )
                if kerning.x != 0:
                    glyph.kerning[g.charcode] = kerning.x / (64.0 * 64.0)
                kerning = face.get_kerning(
                    charcode, g.charcode, mode=FT_KERNING_UNFITTED
                )
                if kerning.x != 0:
