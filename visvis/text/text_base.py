...
def correctVertices(textObject, vertices, charHeigh):
    ...
    # set anchor
    if textObject.halign < 0:
        anchorx = x1
    elif textObject.halign > 0:
        anchorx = x2
    else:
        anchorx = x1 + (x2 - x1) / 2.0
    #
    if textObject.valign < 0:
        anchory = y1
    elif textObject.valign > 0:
        anchory = y2
    else:
        anchory = y1 + (y2 - y1) / 2.0
...
class Text(Wobject, BaseText):
    ...
    def __init__(
        self, parent, text="", x=0, y=0, z=0, fontName=None, fontSize=9, color="k"
    ):
    ...
    def OnDrawScreen(self):
        fm = self._GetFontManager()
        if fm:
            fm.Draw(self, self._screenx, self._screeny, depthToZ(self._screenz))
