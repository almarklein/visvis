    ...
    def aa():
        ...
        def fset(self, value):
            ...
            if not isinstance(value, (int, float)):
                ...
                # Get aa program part
                M = [
                    shaders.SH_2F_AASTEPS_0,
                    shaders.SH_2F_AASTEPS_1,
                    shaders.SH_2F_AASTEPS_2,
                    shaders.SH_2F_AASTEPS_3,
                ]
                aa_steps = M[self._aa]
                # Apply
...
class Texture3D(BaseTexture):
    ...
    def OnDraw(self, fast=False):
        ...
        if self.shader.isUsable and self.shader.hasCode:
            ...
            if (not ok) and (self.renderStyle != "mip"):
                print(
                    "Texture3D detected shader problem; "
            ...
            for iQuad in range(int(len(tex_coord1) / 4)):
                ...
                for i1 in range(4):
                    for i2 in range(4):
                        i3 = (i1 + i2) % 4
                        tex_coord2.append(
                            0.5 * (tex_coord1[io + i1] + tex_coord1[io + i3])
                        )
                        ver_coord2.append(
                            0.5 * (ver_coord1[io + i1] + ver_coord1[io + i3])
                        )
    ...
    def _DrawQuads(self):
        ...
        if (
            (not self._quads)
            or (self._daspectStored != axes.daspect)
            or (self._qcountStored != self._quadPartitionCount(axes.camera))
        ):
            self._CreateQuads()
