...
class TextureObject(object):
    ...
    def _SetDataNow(self):
        ...
        if gl.glIsTexture(self._texId) and self._shape and (data.shape == self._shape):
            # We can update.
            ...
            if count and not ok:
                raise MemoryError(
                    "Could not upload texture to OpenGL, "
                    + "even after 8 times downsampling."
                )
            elif count:
                print(
                    "Warning: data was downscaled "
                    + str(count)
    ...
    def _UpdateTexture(self, data, internalformat, format, gltype):
        ...
        # define dict
        D = {
            1: (gl.glTexSubImage1D, gl.GL_TEXTURE_1D),
    ...
    def _TestUpload(self, data, internalformat, format, gltype):
        ...
        # define dict
        D = {
            1: (gl.glTexImage1D, gl.GL_PROXY_TEXTURE_1D),
    ...
    def _UploadTexture(self, data, internalformat, format, gltype):
        ...
        # define dict
        D = {
            1: (gl.glTexImage1D, gl.GL_TEXTURE_1D),
