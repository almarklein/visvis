...
class BaseCamera(object):
    ...
    def GetViewParams(self):
        ...
        """
        return dict([(key, getattr(self, key)) for key in self.__class__._viewparams])
...
class ThreeDCamera(BaseCamera):
    ...
    def OnMotion(self, event):
        ...
        if constants.KEY_SHIFT in event.modifiers and self._ref_but == 1:
            ...
            # calculate translation
            sro, saz, sel = list(
                map(sind, (self._view_ro, self._view_az, self._view_el))
            )
            cro, caz, cel = list(
                map(cosd, (self._view_ro, self._view_az, self._view_el))
            )
            dx = (
                distx * (cro * caz + sro * sel * saz)
                + distz * (sro * caz - cro * sel * saz)
            ) / ar[0]
            dy = (
                distx * (cro * saz - sro * sel * caz)
                + distz * (sro * saz + cro * sel * caz)
            ) / ar[1]
            ...
            if self.axes.daspectAuto:
                ...
                dx = -factorx * abs(cro * caz + sro * sel * saz) + factory * abs(
                    sro * caz - cro * sel * saz
                )
                dy = -factorx * abs(cro * saz - sro * sel * caz) + factory * abs(
                    sro * saz + cro * sel * caz
                )
...
class FlyCamera(BaseCamera):
    ...
    def OnTimer(self, event):
        ...
        if True:
            acc = 0.2
            #
            if self._acc_forward > 0:
                self._speed_forward += acc
            elif self._speed_forward > 0:
                self._speed_forward -= acc * 0.5
            if self._acc_forward < 0:
                self._speed_forward -= acc
            elif self._speed_forward < 0:
                self._speed_forward += acc * 0.5
            #
            if self._acc_right > 0:
                self._speed_right += acc
            elif self._speed_right > 0:
                self._speed_right -= acc * 0.5
            if self._acc_right < 0:
                self._speed_right -= acc
            elif self._speed_right < 0:
                self._speed_right += acc * 0.5
            ...
            if self._acc_up > 0:
                self._speed_up += acc
            elif self._speed_up > 0:
                self._speed_up -= acc * 0.5
            if self._acc_up < 0:
                self._speed_up -= acc
            elif self._speed_up < 0:
                self._speed_up += acc * 0.5
            #
            if self._acc_pitch > 0:
                self._speed_pitch += acc
            elif self._speed_pitch > 0:
                self._speed_pitch -= acc * 0.5
            if self._acc_pitch < 0:
                self._speed_pitch -= acc
            elif self._speed_pitch < 0:
                self._speed_pitch += acc * 0.5
            ...
            if self._acc_yaw > 0:
                self._speed_yaw += acc
            elif self._speed_yaw > 0:
                self._speed_yaw -= acc * 0.5
            if self._acc_yaw < 0:
                self._speed_yaw -= acc
            elif self._speed_yaw < 0:
                self._speed_yaw += acc * 0.5
            #
            if self._acc_roll > 0:
                self._speed_roll += acc
            elif self._speed_roll > 0:
                self._speed_roll -= acc * 0.5
            if self._acc_roll < 0:
                self._speed_roll -= acc
            elif self._speed_roll < 0:
                self._speed_roll += acc * 0.5
            ...
            if abs(self._speed_forward) < acc:
                self._speed_forward = 0
            if abs(self._speed_right) < acc:
                self._speed_right = 0
            if abs(self._speed_up) < acc:
                self._speed_up = 0
            if abs(self._speed_pitch) < acc:
                self._speed_pitch = 0
            if abs(self._speed_yaw) < acc:
                self._speed_yaw = 0
            if abs(self._speed_roll) < acc:
                self._speed_roll = 0
        ...
        if True:
            if abs(self._acc_forward) == 1:
                self._acc_forward = 0
            if abs(self._acc_right) == 1:
                self._acc_right = 0
            if abs(self._acc_up) == 1:
                self._acc_up = 0
            if abs(self._acc_pitch) == 1:
                self._acc_pitch = 0
            if abs(self._acc_yaw) == 1:
                self._acc_yaw = 0
            if abs(self._acc_roll) == 1:
                self._acc_roll = 0
        ...
        if not (
            self._speed_forward
            or self._speed_right
            or self._speed_up
            or self._speed_pitch
            or self._speed_yaw
            or self._speed_roll
            or magnitude
        ):
