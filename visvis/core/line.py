...
class Sprite:
    ...
    def Create(self):
        ...
        self._canUse = getOpenGlCapable("2.0", "point sprites (for advanced markers)")
        if not self._canUse:
            return
...
class PolarLine(Line):
    ...
    def _GetPolarLimits(self):
        if not self._points:
            ...
            return Range(self._angs.min(), self._angs.max()), Range(
                self._mags.min(), self._mags.max()
            )
