...
class Shader(object):
    ...
    def _CheckUniform(self, name, value):
        ...
        if not (
            ...
            or (isinstance(value, (list, tuple)) and len(value) < 5)
            or (isinstance(value, vv.core.baseTexture.TextureObject))
        ):
...
class GlslProgram:
    ...
    def IsUsable(self):
        ...
        if self._usable is None:  # Not True nor False
            usable = getOpenGlCapable(
                "2.0", "anti-aliasing, the clim property, colormaps and 3D rendering"
            )
    ...
    def DestroyGl(self):
        ...
        if self._programId > 0:
            try:
                gla.glDeleteObjectARB(self._programId)
            except Exception:
                pass
        for shaderId in self._shaderIds:
            try:
                gla.glDeleteObjectARB(shaderId)
            except Exception:
                pass
        # reset
        self._programId = 0
...
class ShaderCode(object):
    ...
    def _IndexOfPart(self, part):
        ...
        if isinstance(part, basestring):
            ...
            raise ValueError(
                "Inalid part description (must be string or ShaderCodePart)."
            )
