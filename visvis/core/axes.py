...
class AxesContainer(Box):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        if not isinstance(parent, _BaseFigure):
            raise Exception(
                "The given parent for an AxesContainer " + "should be a Figure."
            )
...
class Axes(base.Wibject):
    ...
    def __init__(self, parent, axisClass=None):
        ...
        if isinstance(parent, AxesContainer):
            ...
            raise Exception(
                "The given parent for an Axes " + "should be a Figure or AxesContainer."
            )
    ...
    def _OnDrawInMode(self, mode, bgcolor, pickerHelper=None):
        ...
        if self._useBuffer and fig.enableUserInteraction:
            ...
            canUseScreenshot = (
                (sshot is not None)
                and sshot.shape[0] == pos.h
                and sshot.shape[1] == pos.w
            )

            # Test if we want to blur with the screenshot
            blurWithScreenshot = (
                bool(self._motionBlur) and self._isdirty and mode == DRAW_FAST
            )
