...
class CartesianAxis3D(BaseAxis):
    ...
    def _CreateLinesAndLabels(self, axes):
        ...
        for d in range(3):  # d for dimension/direction
            ...
            for i in range(4):
                if d == 2:
                    val = corners4_s[i].x  # chose leftmost corner
                else:
                    val = corners4_s[i].y  # chose bottommost corner
                if val < bestVal:
                    i0 = i
                    bestVal = val
...
class PolarAxis2D(BaseAxis):
    ...
    def _CreateLinesAndLabels(self, axes):
        ...
        for tick, pos, text in zip(ticks, ticksPos, ticksText):
            ...
            p2 = Point(
                (textRadius * np.cos(theta))[0], (textRadius * np.sin(theta))[0], 0
            )
            # Put a textlabel at tick
        ...
        for tick, pos, qIndx in zip(ticks, ticksPos, quadIndex):
            ...
            if tickXformed in textDict and textDict[tickXformed] in self._children:
    ...
    def isCW():
        ...
        def fset(self, value):
            if isinstance(value, bool):
                ...
                raise Exception(
                    "isCW can only be assigned " + "by a bool (True or False)"
                )
