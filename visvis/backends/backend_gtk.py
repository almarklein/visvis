...
MOUSEMAP = {
    gtk.gdk.BUTTON_PRESS: "down",
    gtk.gdk.BUTTON_RELEASE: "up",
    gtk.gdk._2BUTTON_PRESS: "double",
}

KEYMAP = {
    gtk.keysyms.Shift_L: constants.KEY_SHIFT,
    ...
    def _on_button_event(self, widget, event):
        if event.type in MOUSEMAP:
            ...
            self.figure._GenerateMouseEvent(
                MOUSEMAP[event.type], event.x, event.y, button, modifiers(event)
            )

    def _on_scroll_event(self, widget, event):
        horizontal = {gtk.gdk.SCROLL_LEFT: 1.0, gtk.gdk.SCROLL_RIGHT: -1.0}.get(
            event.direction, 0
        )
        vertical = {gtk.gdk.SCROLL_UP: 1.0, gtk.gdk.SCROLL_DOWN: -1.0}.get(
            event.direction, 0
        )
        if horizontal or vertical:
            self.figure._GenerateMouseEvent(
                "scroll", event.x, event.y, horizontal, vertical, modifiers(event)
            )

    def _on_key_press_event(self, widget, event):
        self.figure._GenerateKeyEvent(
            "keydown",
            KEYMAP.get(event.keyval, event.keyval),
            event.string,
            modifiers(event),
        )
