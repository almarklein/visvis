...
def _not_equal(ob1, ob2):
    ...
    if isstruct(ob1) or isinstance(ob1, dict):
        ...
        # Test properties
        if not (
            ob1.shape == ob2.shape and ob1.dtype == ob2.dtype and ob1.data == ob2.data
        ):
        ...
        if not (
            ob1.shape == ob2.shape
            and ob1.dtype == ob2.dtype
            and (ob1 == ob2).sum() == ob1.size
        ):
            return "<array does not match>"
...
class Block:
    ...
    def from_object(cls, indent, name, value):
        ...
        if value is None:
            ...
            if name is not None:
                print(
                    "SSDF: %s is unknown object: %s %s" % (name, tmp, repr(type(value)))
                )
            else:
                print("SSDF: unknown object: %s %s" % (tmp, repr(type(value))))
