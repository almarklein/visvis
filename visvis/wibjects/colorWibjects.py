...
class ColormapEditor(BaseMapableEditor):
    ...
    def __init__(self, parent, *args):
        ...
        # Create buttons
        self._rBut = RadioButton(self, "R")
        self._rBut.position = 5, 35, 12, 14
        self._gBut = RadioButton(self, "G")
        self._gBut.position = 20, 35, 12, 14
        self._bBut = RadioButton(self, "B")
        self._bBut.position = 35, 35, 12, 14
        self._aBut = RadioButton(self, "A")
        self._aBut.position = 5, 52, 12, 14
...
class Colorbar(Box):
    ...
    def _OnPositionChange(self, event):
        ...
        if w > h:
            ...
            self._label.position.x = (w - 100) / 2.0
            ...
            self._label.position.y = (h - 100) / 2.0
    ...
    def OnDraw(self):
        ...
        if mapable:
            ...
            gl.glTexCoord1f(texCords[0])
            gl.glVertex2f(0, 0)
            gl.glTexCoord1f(texCords[1])
            gl.glVertex2f(0, h)
            gl.glTexCoord1f(texCords[2])
            gl.glVertex2f(w, h)
            gl.glTexCoord1f(texCords[3])
            gl.glVertex2f(w, 0)
            gl.glEnd()
        ...
        if hasattr(mapable, "clim"):
            ...
            for tick, pos, text in zip(ticks, ticksPos, ticksText):
                ...
                # Add line piece
                linePieces.append(pos)
                linePieces.append(pos2)
