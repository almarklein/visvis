...
class WavefrontReader(object):
    ...
    def readFace(self, line):
        ...
        for indexSet in indexSets:
            ...
            if self._texcords is not None:
                if len(indices) > 1 and indices[1]:
                    ...
                    if self._texcords:
                        print(
                            "Warning reading OBJ: ignoring texture coordinates because "
            ...
            if self._normals is not None:
                if len(indices) > 2 and indices[2]:
                    ...
                    if self._normals:
                        print(
                            "Warning reading OBJ: ignoring normals because it is not "
        ...
        if self._faces and len(self._faces[0]) != len(final_face):
            raise RuntimeError(
                "Visvis requires that all faces are either triangles or quads."
            )
